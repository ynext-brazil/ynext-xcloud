...
impl InputReport {
    ...
    pub fn from_gamepad(gamepad: &Gamepad, index: u8) -> Self {
        ...
        // D-PAD
        if gamepad.is_pressed(Button::DPadUp) { report.buttons |= 0x0001; }
        if gamepad.is_pressed(Button::DPadDown) { report.buttons |= 0x0002; }
        if gamepad.is_pressed(Button::DPadLeft) { report.buttons |= 0x0004; }
        if gamepad.is_pressed(Button::DPadRight) { report.buttons |= 0x0008; }
        // Start / Back
        if gamepad.is_pressed(Button::Start) { report.buttons |= 0x0010; }
        if gamepad.is_pressed(Button::Select) { report.buttons |= 0x0020; }
        // Thumbs
        if gamepad.is_pressed(Button::LeftThumb) { report.buttons |= 0x0040; }
        if gamepad.is_pressed(Button::RightThumb) { report.buttons |= 0x0080; }
        // Bumpers
        if gamepad.is_pressed(Button::LeftTrigger) { report.buttons |= 0x0100; } // LB
        if gamepad.is_pressed(Button::RightTrigger) { report.buttons |= 0x0200; } // RB
        // ABXY
        if gamepad.is_pressed(Button::South) { report.buttons |= 0x1000; } // A
        if gamepad.is_pressed(Button::East) { report.buttons |= 0x2000; }  // B
        if gamepad.is_pressed(Button::West) { report.buttons |= 0x4000; }  // X
        if gamepad.is_pressed(Button::North) { report.buttons |= 0x8000; } // Y
